...
class GFQuiz extends GFAddOn {
	...
	public function scripts() {
		...
		$scripts = array(
			array(
				...
				'src'      => $this->get_base_url() . "/js/gquiz_form_editor{$min}.js",
			...
			array(
				...
				'src'      => $this->get_base_url() . "/js/gquiz_form_settings{$min}.js",
				'version'  => $this->_version,
	...
	public function styles() {
		...
		$styles = array(
			array(
				...
				'src'     => $this->get_base_url() . "/css/gquiz_form_editor{$min}.css",
				'version' => $this->_version,
				'enqueue' => array(
					array( 'admin_page' => array( 'form_editor' ) ),
				)
			...
			array(
				...
				'src'     => $this->get_base_url() . "/css/gquiz_form_settings{$min}.css",
				'version' => $this->_version,
				'enqueue' => array(
			...
			array(
				'handle'  => 'gquiz_css',
				'src'     => $this->get_base_url() . "/css/gquiz{$min}.css",
				'version' => $this->_version,
				'enqueue' => array(
	...
	public function results_markup( $html, $data, $form, $fields ) {
		...
		if ( $entry_count > 0 ) {
			...
			$html .= $this->get_score_frequencies_chart( rgar( $data, 'score_frequencies', array() ) );

			if ( $grading == 'letter' ) {
				$html .= "<div class='gresults-results-field-label'>" . esc_html__( 'Grade Frequencies', 'gravityformsquiz' ) . "</div>";
				$html .= "<div class='gquiz-results-grades'>" . $this->get_grade_frequencies_chart( rgar( $data, 'grade_frequencies', array() ) ) . '</div>';
			}
			...
			foreach ( $fields as $field ) {
				...
				$html .= '<div>' . $this->get_field_score_results( $field, rgars( $field_data, $field_id . '/totals/correct', 0 ), $entry_count ) . '</div>';
			...
			if ( $field->gquizEnableRandomizeQuizChoices ) {
				...
				for ( $i = $nodes->length - 1; $i >= 0; $i -- ) {
					...
					if ( $i <> $n ) {
                        ...
                        // select all option for checkboxes orss select one placeholder for dropdown.
						$no_shuffle_strings = array( 'gchoice_select_all', 'gf_placeholder');
